...
export class State {
    ...
    markReviewAsPublished(appId: string, reviewId: string): void {
        ...
        if (this.common.verbose) {
            console.log(`INFO: Checking if we need to prune published reviews have (${length}) limit (${Constants.REVIEWS_LIMIT})`);
        }
        ...
        if (this.common.verbose) {
            console.log("INFO: Review marked as published: " + JSON.stringify(appState));
        }
