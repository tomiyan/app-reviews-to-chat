...
export class AppStore {
        ...
        for (const region of regions) {
            ...
            if (this.state.isFirstRun()) {
                ...
                if (this.commonConf.dryRun && reviews.length > 0) {
                    ...
                    await this.publishReview(region, appInformation, reviews[reviews.length - 1], this.commonConf.dryRun);
                }
            } else {
                await this.handleFetchedAppStoreReviews(region, appInformation, reviews);
            }
        }
    }
    ...
    private async fetchAppInformation(): Promise<AppInformation> {
        ...
        try {
            ...
            const entries: any[] = data.results;
            if (entries == null || entries.length === 0) {
                if (this.commonConf.verbose) {
                    console.log(`INFO: Received no data from App Store for (${this.appStoreConf.appId})`);
                }
                return await Promise.reject(new Error(`INFO: Received no data from App Store for (${this.appStoreConf.appId})`));
            }
            if (this.commonConf.verbose) {
                console.log(`INFO: Received data from App Store for (${this.appStoreConf.appId})`);
            }
            ...
            if (this.commonConf.verbose) {
                console.log(`ERROR: Error fetching app data from App Store for (${this.appStoreConf.appId})`);
                console.log(error);
            }
    ...
    private async fetchAppStoreReviews(region: string, appInformation: AppInformation): Promise<Data[]> {
        let allReviews: Data[] = [];
        for (let page = 1; page < 10; page++) {
            const reviews = await this.fetchAppStoreReviewsByPage(region, appInformation, page);
            if (reviews.length === 0) {
    ...
    private async fetchAppStoreReviewsByPage(region: string, appInformation: AppInformation, page: number): Promise<Data[]> {
        ...
        try {
            ...
            if (entries == null || entries.length === 0) {
                if (this.commonConf.verbose) {
                    console.log(`INFO: Received no reviews from App Store for (${this.appStoreConf.appId}) (${region})`);
                }
                return await Promise.resolve([]);
            }
            console.log(`INFO: Received reviews from App Store for (${this.appStoreConf.appId}) (${region})`);
            const that = this;
            const reviews = entries
                .filter(function (review: any) {
                    return !review.hasOwnProperty('im:name');
                })
                .reverse()
            ...
            if (this.commonConf.verbose) {
                console.log(`ERROR: Error fetching reviews from App Store for (${this.appStoreConf.appId}) (${region})`);
                console.log(error)
            }
    ...
    private parseAppStoreReview(region: string, rssItem: any, appInformation: AppInformation): Data {
        return {
            id: rssItem.id.label,
    ...
    private async publishReview(region: string, appInformation: AppInformation, review: Data, force: boolean) {
    ...
    private async handleFetchedAppStoreReviews(region: string, appInformation: AppInformation, reviews: Data[]) {
        if (this.commonConf.verbose) {
            console.log(`INFO: [${this.appStoreConf.appId} (${region})] Handling fetched reviews`);
        }
