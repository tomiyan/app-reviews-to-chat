...
export class AppInformation {
  constructor(readonly appName: string, readonly appIcon: string) {}
}
...
export class GooglePlay {
  ...
  private async fetchAppInformation(): Promise<AppInformation> {
    try {
      ...
      if (this.commonConf.verbose) {
        console.log(
          `ERROR: Error fetching app data from Google Play for (${this.appStoreConf.appId})`,
        );
        console.log(error);
      }
  ...
  private async fetchAppStoreReviews(
    ...
    const tokens = new google.auth.GoogleAuth({
      scopes: scopes,
    ...
    try {
      ...
      for (const review of response.data.reviews) {
        ...
        allReviews.push({
          id: review.reviewId!,
          version: comment?.appVersionName!, // eslint-disable-line @typescript-eslint/no-non-null-asserted-optional-chain
          osVersion: osVersion,
          device: device,
          title: undefined,
      ...
      console.error(
        `ERROR: [${this.appStoreConf.appId}] Could not fetch Google Play reviews, ${error}`,
      );
      return Promise.reject(error);
    }
  ...
  private async handleFetchedAppStoreReviews(
    ...
    if (this.commonConf.verbose) {
      console.log(
        `INFO: [${this.appStoreConf.appId}] Handling fetched reviews`,
      );
    }
    for (const review of reviews) {
